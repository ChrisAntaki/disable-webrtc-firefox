...
NOTE: Disabling WebRTC will break some sites that depend on it, like Google Voice. Firefox doesn't allow us to disable WebRTC per-tab or per-window either. The setting applies to the entire browser. So if you need to use WebRTC sometimes, "the best solution for the foreseeable future is to either have another browser (perhaps firefox developer edition without this extension) used for those services, or enable WebRTC, and disable it again once finished." - [karlbright@](https://github.com/ChrisAntaki/disable-webrtc-firefox/issues/27#issuecomment-393377906)
